...
{
    float selected_rect; // or other relevant members
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (it_anim == anim.end())
    {
        anim.emplace(id, checkbox_element{ 0.0f }); // Use emplace or insert
    ...
    // Define colors
    ImU32 active_color = ImColor(0, 255, 255, (int)(it_anim->second.selected_rect * 255)); // Lime green
    ImU32 hovered_color = ImColor(173, 255, 47, (int)(it_anim->second.selected_rect * 255)); // Yellow green

    // Draw checkbox based on state
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(32, 33, 35), style.FrameRounding); // Background color
    if (*v)
    {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, active_color, style.FrameRounding);
    }
    else if (hovered)
    {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, hovered_color, style.FrameRounding);
    }

    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(4, 4), ImColor(1.0f, 1.0f, 1.0f, it_anim->second.selected_rect), square_sz - 8.0f);
